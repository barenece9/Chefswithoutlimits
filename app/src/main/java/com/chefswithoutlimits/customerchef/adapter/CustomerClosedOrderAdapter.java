...
public class CustomerClosedOrderAdapter extends BaseAdapter {
	...
	public CustomerClosedOrderAdapter(Context mainActivity, ArrayList<HashMap<String,String>> listitems1) {
	...
	{
		CardView ll_bg;
		TextView invoiceno,secrete_key,total_amont,datetime_added,order_type,schedule_date,
				tn_check,item_details,item_details_bill;
		EditText etn_code;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		View rowView;
		rowView = inflater.inflate(R.layout.infleter_customer_complete_order, null);
		...
		if(order_status.equalsIgnoreCase("draft")){
			...
			holder.invoiceno.setTextColor(Color.parseColor("#ffffff"));
			holder.secrete_key.setTextColor(Color.parseColor("#ffffff"));
			holder.total_amont.setTextColor(Color.parseColor("#ffffff"));
			holder.datetime_added.setTextColor(Color.parseColor("#ffffff"));
			holder.order_type.setTextColor(Color.parseColor("#ffffff"));
			holder.schedule_date.setTextColor(Color.parseColor("#ffffff"));

		}else if(order_status.equalsIgnoreCase("delivered")){
			//holder.ll_bg.setBackgroundColor(Color.parseColor("#1dc499"));green
			holder.ll_bg.setBackgroundColor(Color.parseColor("#EEE1D5"));//gray
			//holder.schedule_date.setVisibility(View.GONE);
			holder.etn_code.setVisibility(View.GONE);
			holder.tn_check.setVisibility(View.GONE);

			holder.invoiceno.setTextColor(Color.parseColor("#00000c"));
			holder.secrete_key.setTextColor(Color.parseColor("#00000c"));
			holder.total_amont.setTextColor(Color.parseColor("#00000c"));
			holder.datetime_added.setTextColor(Color.parseColor("#00000c"));
			holder.order_type.setTextColor(Color.parseColor("#00000c"));
			holder.schedule_date.setTextColor(Color.parseColor("#00000c"));
		...
		holder.tn_check.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!code.equalsIgnoreCase("")) {
					...
					if (context instanceof CustomerCompleteOrderActivity) {
						//((CustomerCompleteOrderActivity) context).CheckSecrateCode(code);
					}
				}else {

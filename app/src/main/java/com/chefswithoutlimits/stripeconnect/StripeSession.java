...
public class StripeSession {
	...
	private static  SharedPreferences sharedPref2;
	...
	public StripeSession(Context context, String accountName) {
		...
		sharedPref2 = context.getSharedPreferences(accountName + SHARED, Context.MODE_PRIVATE);
	}
	...
	public static void Stripelogout(){
		editor2 = sharedPref2.edit();
		editor2.remove(API_ACCESS_TOKEN);
		editor2.remove(API_REFRESH_TOKEN);
		editor2.remove(API_PUBLISHABLE_KEY);
		editor2.remove(API_USER_ID);
		editor2.remove(API_TOKEN_TYPE);
		editor2.remove(API_LIVE_MODE);
		editor2.clear();
		editor2.commit();
	}
